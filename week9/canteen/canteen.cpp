...
void testcase() {
    ...
    const int MAX_PROFIT = 20; // Given in the pdf
    ...
    for (int i = 0 ; i < n ; i++) {
        ...
        adder.add_edge(i, sink, si, MAX_PROFIT - pi); // - pi because the price students pay reduce the cost by that much
    ...
    cout << (flow == hungry_students ? "possible" : "impossible") << " " << flow << " " << profit << endl;
}
