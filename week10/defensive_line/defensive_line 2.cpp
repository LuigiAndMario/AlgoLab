...
vector<vector<int> > memo; // [attackers_left][prev] -> value
...
int strategy(int value, int cur, int prev, int attackers_left, bool added_stuff) {
    ...
    // Already computed
    //
    if (memo[attackers_left][prev] != -2) {
        return memo[attackers_left][prev];
    }
    ...
    if (!added_stuff || a_cur > b_prev) {
        ...
        memo[attackers_left][prev] = max(with, without);
    } else {
        // We cannot add this interval - we move onto the next
        int without = strategy(value, cur + 1, prev, attackers_left, added_stuff);
        memo[attackers_left][prev] = without;
    }
...
void testcase() {
    ...
    memo = vector<vector<int> >(m + 1, vector<int>(intervals.size(), -2));
