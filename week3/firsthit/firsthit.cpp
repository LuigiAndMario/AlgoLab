...
void testcase(int n) {
    ...
    int i = 0;
    while (!collision && i < n) {
        if (do_intersect(laser, obstacles[i])) {
            // We have to use the laser for the first intersection - first and second might make a segment that doesn't intersect with anything
            collision = true;
            adjust(trajectory, intersection(laser, obstacles[i]));
        } else {
            i++;
        }
    }
    ...
    if (collision) {
        while (i < n) {
            if (do_intersect(trajectory, obstacles[i])) {
